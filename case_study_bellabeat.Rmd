...
The insights derived from this analysis will be applied to Bellabeat’s product portfolio to identify opportunities for growth, optimize marketing campaigns, and strengthen the company’s position in the global wellness technology market.  
...
2. **Prepare** – Collect and organize the necessary data.  
...
6. **Act** – Apply insights to support business decisions and strategies.  
...
- **What is the problem you are trying to solve?**  
  Bellabeat wants to understand how customers use smart devices (e.g., Fitbit) in order to identify usage trends. The aim is to apply these insights to one of Bellabeat’s products and improve its marketing strategy, ultimately supporting the company’s expansion in the global wellness technology market.  

- **How can your insights drive business decisions?**  
  The insights will highlight customer behavior patterns (e.g., activity levels, sleep tracking, daily steps). These findings can inform targeted marketing campaigns, refine product positioning, and identify growth opportunities by aligning Bellabeat’s offerings with what users value most in wellness technology.  
...
2. **Consider key stakeholders**  
   - **Urška Sršen (Cofounder & CCO):** Wants actionable insights that align with Bellabeat’s brand vision.  
...
- **Does your data ROCCC?**  
  - **Reliable:** Limited reliability; users logged different durations, and demographic data (e.g., age, gender) is missing. Additionally, not all users logged all activities—some only tracked steps, while others only tracked sleep or heart rate.  
  ...
  - **Cited:** Provided by Möbius and available on Kaggle under CC0 license.  
...
1. **Download the data** → Retrieved from Kaggle and stored locally in organized folders.  
2. **Unzip and review files** → Inspected CSVs to understand their structure and variables.  
3. **Filter the data** → Identified datasets relevant to the business task.  
4. **Determine credibility** → Evaluated the dataset using the ROCCC framework.  

#### Selected Datasets (to be used)  

- *dailyActivity_merged.csv* → Summary of daily steps, distance, calories, and activity minutes.  
- *sleepDay_merged.csv* → Sleep records including duration and efficiency.  
- *hourlySteps_merged.csv* → Hourly step counts, indicating movement patterns.  
- *weightLogInfo_merged.csv* (optional) → Weight and BMI logs.  
...
- *heartrate_seconds_merged.csv* → Too granular; second-level data not necessary for current business scope.  
- *minute datasets (e.g., minuteStepsNarrow.csv, minuteCaloriesWide.csv)* → High granularity, may add noise without significant value.  
- *Duplicated daily-level files* → Redundant, already captured in dailyActivity_merged.csv.  
...
# Remove WeightPounds (we’ll keep kg version)
weight_log_info <- weight_log_info %>%
  select(-WeightPounds)
...
- Imported and consolidated datasets from March–April and April–May.  
...
- Removed duplicates and inconsistencies → ensured higher data quality.  
...
- Users’ activity aligns with the common 10,000-step health goal.
- However, many days fall under 5,000 steps → sedentary or inactive users.
...
- Users’ walking habits remain consistent regardless of the day of the week.
...
- The median is around 2,200–2,300 calories.
...
- Most data points fall between ~2,000 and 15,000 steps, corresponding to ~1,500–3,000 calories burned.
...
- Active users (5,000–10,000 steps/day) tend to maintain calorie burn within the healthy range of ~2,000–3,000.
...
- Bellabeat can encourage users to aim for 5,000–10,000 steps per day as a sustainable and healthy activity habit.
...
- From mid-April onward, users consistently reach ~7,000–8,000 steps per day.
...
- The stabilization near 7,000–8,000 steps indicates a sustainable average for many users.
...
- On weekends, users sleep ~7.3–7.7 hours with slightly lower efficiency (91.0%).
...
- Bellabeat can promote consistent sleep routines (“consistency over quantity”).
...
- The majority of nights fall between 350 and 500 minutes of sleep (≈ 6–8 hours).
- The peak is around 420–450 minutes (~7 hours).
...
- On weekends, median sleep duration rises to 7.5–8 hours (450–480 min).
...
- On weekends, they compensate by sleeping longer, a common “sleep debt” pattern.
...
- Sleep hygiene programs can be targeted at users with large weekday–weekend gaps to reduce long-term sleep debt.
...
- Most users (blue points) cluster around 350–500 minutes of sleep with efficiency >85%.
...
- The average BMI is 25.4, which falls in the overweight range (≥25).
...
- The relatively moderate body fat percentage may indicate that not all excess BMI is due to unhealthy fat mass—it could be lifestyle factors (diet, sedentary behavior, lack of activity).
...
- The top weight gainers increased their weight by up to 3.9 kg, with smaller gains around 0.4–0.6 kg.
...
- **Position Bellabeat as a Proactive Partner**: Position Bellabeat not just as a data logger, but as a personal wellness coach. The app’s features for sleep debt alerts and tailored nudges demonstrate a proactive approach to a user’s well-being.

- **Align with Long-Term Habits**: Focus on promoting sustainable, long-term habits rather than short-term gains. By encouraging a consistent 5,000–10,000 steps per day, Bellabeat can help users maintain a healthy routine
